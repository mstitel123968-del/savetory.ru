...
class SeoPagesTests(TestCase):
    def test_landing_has_required_seo_and_visible_copy(self):
        ...
        self.assertContains(response, '<title>СКлад — онлайн-каталог для вашей коллекции</title>', html=True)
        self.assertContains(response, '<h1 id="landing-title">Ваша коллекция.<br>В одном месте.</h1>', html=True)
        self.assertContains(response, 'Сохраняйте фотографии и информацию о важных предметах')
        self.assertContains(response, 'data-auth-tab="login"')
        self.assertContains(response, 'data-auth-tab="register"')
        self.assertContains(response, 'href="#collection-example"')
